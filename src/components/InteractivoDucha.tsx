...
const InteractivoDucha: React.FC = () => {
    ...
    if (showDressUpStage) {
      ...
      setClothes((prevClothes) => [
        ...
        ...removedClothes.map((item) => {
          const position = leftSidePositions.find(
            (pos) => pos.key === item.key,
          );
          return {
            ...item,
            style: {
  ...
  const [fixedClothes, setFixedClothes] = useState<
    Record<ClothingKey, boolean>
  >({
    shirt: false,
    pants: false,
    socks: false,
  ...
  return (
    <View style={styles.container}>
      <GameFinished
        ...
        onRetry={() => router.back()} // TODO: Add retry functionality
      ...
      {!showShowerStage && !showSoapStage && !showDressUpStage ? (
        ...
        <>
          ...
          <TouchableOpacity
            onPress={handleShowerPress}
            disabled={!canTurnOnShower}
          >
            <Image
              source={
                showerOn
          ...
          {showerOn && <Text style={styles.message}>¡Ya estás limpio! 🎉</Text>}
        </>
      ) : (
        <>
          <Text style={styles.instructions}>
          ...
          {clothes.map((item) =>
            fixedClothes[item.key] ? (
              <View
                key={item.key}
                style={[styles.fixedClothingContainer, item.style]}
              >
                <Image source={item.source} style={styles.fixedClothingImage} />
              </View>
            ) : (
              <DraggableItem
                key={item.key}
